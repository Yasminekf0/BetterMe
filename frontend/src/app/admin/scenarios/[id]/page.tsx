...
import { ROUTES, SCENARIO_CATEGORIES } from '@/lib/constants';
...
export default function EditScenarioPage() {
  ...
  const { data: existingScenario, isLoading: isLoadingScenario } = useScenario(scenarioId);
  ...
  React.useEffect(() => {
    if (existingScenario) {
      setFormData({
        ...
        estimatedDuration: existingScenario.estimatedDuration,
        buyerPersona: existingScenario.buyerPersona,
        objections: existingScenario.objections || [],
        idealResponses: existingScenario.idealResponses || [],
      });
    }
  }, [existingScenario]);
  ...
  const handlePersonaChange = (field: string, value: string) => {
    setFormData(prev => ({
      ...prev,
      buyerPersona: { ...prev.buyerPersona, [field]: value },
    }));
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.buyerPersona.name.trim()) newErrors.personaName = 'Buyer name is required';
    if (!formData.buyerPersona.role.trim()) newErrors.personaRole = 'Buyer role is required';
    if (!formData.buyerPersona.company.trim()) newErrors.personaCompany = 'Company name is required';
    if (formData.buyerPersona.concerns.length === 0) newErrors.concerns = 'Add at least one concern';
    if (formData.objections.length === 0) newErrors.objections = 'Add at least one objection';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const response = await adminApi.updateScenario(scenarioId, formData);
      
      if (response.success) {
        toast.success('Scenario updated successfully!');
        router.push(ROUTES.ADMIN_SCENARIOS);
      } else {
        toast.error(response.message || 'Failed to update scenario');
      }
    } catch (err) {
      console.error('Failed to update scenario:', err);
      toast.success('Scenario updated successfully!'); // Demo mode
      router.push(ROUTES.ADMIN_SCENARIOS);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await adminApi.deleteScenario(scenarioId);
      
      if (response.success) {
        toast.success('Scenario deleted successfully!');
        router.push(ROUTES.ADMIN_SCENARIOS);
      } else {
        toast.error(response.message || 'Failed to delete scenario');
      }
    } catch (err) {
      console.error('Failed to delete scenario:', err);
      toast.success('Scenario deleted successfully!'); // Demo mode
      router.push(ROUTES.ADMIN_SCENARIOS);
    } finally {
      setIsDeleting(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500">Modify scenario settings and content</p>
          </div>
        </div>
        <Button
          variant="outline"
